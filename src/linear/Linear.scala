...
class Linear(
  ...
  val io = IO(new Bundle {
    ...
    val y = Output(Vec(outFeatures, SInt(outputWidth.W)))
  })
  for (j <- 0 until outFeatures) {
    ...
    val acc = products.reduce(_ + _) + io.bias(j)
    val requantize = Module(new Requantize(accWidth, outputWidth, shift))
    requantize.io.in := acc
    io.y(j) := requantize.io.out
  }
}
